...
class PortalAccount(CustomerPortal):
    @http.route(
        ["/my/invoices/<int:invoice_id>/accept"],
        type="json",
        auth="public",
        website=True,
    )
    def portal_invoice_accept(
        self,
        res_id,
        access_token=None,
        partner_name=None,
        ...
        if not invoice_sudo.has_to_be_signed(also_in_draft=True):
            return {
                "error": _(
                    "Invoice is not in a state requiring customer signature."
                )
            }
        if not signature:
        ...
        pdf = (
            ...
            .sudo()
            .render_qweb_pdf([invoice_sudo.id])[0]
        )
    ...
    @http.route(
        ["/my/invoices/<int:invoice_id>/decline"],
        type="http",
        auth="public",
        methods=["POST"],
        website=True,
    ...
    def decline(self, invoice_id, access_token=None, *args, **post):
        try:
            invoice_sudo = self._document_check_access(
                "account.invoice", invoice_id, access_token=access_token
            )
        except (AccessError, MissingError):
        ...
        return request.redirect(
            invoice_sudo.get_portal_url(query_string=query_string)
        )
