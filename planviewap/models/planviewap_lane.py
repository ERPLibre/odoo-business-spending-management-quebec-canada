...
class PlanViewAPLane(models.Model):
    _name = "planviewap.lane"
    _description = "planviewap_lane"
    _order = "name,id"
    ...
    lane_parent_id = fields.Many2one(
        string="Parent Lane", comodel_name="planviewap.lane"
    )
    ...
    def action_sync_cards(self):
        for rec in self:
            self.env["planviewap.card"].sync_pvap_cards(
