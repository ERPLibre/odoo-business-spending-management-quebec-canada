...
class PlanViewAPCard(models.Model):
    _name = "planviewap.card"
    _description = "planviewap_card"
    ...
    card_type_id = fields.Many2one(
        comodel_name="planviewap.card.type",
        string="Card Type",
    )

    lane_id = fields.Many2one(
        comodel_name="planviewap.lane",
        string="Lane",
    ...
    def _default_stage(self):
        return self.env["planviewap.lane"].search([], limit=1)
    ...
    def _read_group_lane_ids(self, stages, domain, order):
        ...
        if not domain:
            return (
                self.env["planviewap.lane"]
                .search([], order=order, limit=80)
                .sorted("name")
            )
        card_ids = self.env["planviewap.card"].search(domain)
        lane_ids = self.env["planviewap.lane"]
    ...
    def sync_pvap_cards(self, board_id, from_lane=None):
        ...
        if not lane_to_extract_ids:
            all_card_ids = self.env["planviewap.card"].search(
        ...
        for dct_card in lst_cards:
            ...
            lane_id = self.env["planviewap.lane"].search(
                [
        ...
        card_to_delete_ids = self.env["planviewap.card"]
