...
class JistiMeet(models.Model):
    ...
    external_participants = fields.One2many(
        "sinerkia_jitsi_meet.external_user",
        "meet",
        string="External Participant",
    )
    ...
    def get_channel(self, jitsi_id):
        if jitsi_id == 0:
            meetings = self.sudo().create(
                {"name": "new from website", "date": fields.Datetime.now()}
            )
        elif jitsi_id > 0:
            meetings = self.sudo().browse(jitsi_id)
        else:
            meetings = self.sudo().search([("name", "!=", None)], limit=1)
    ...
    def create(self, vals):
        ...
        if res.send_mail:
            template = self.env.ref(
                "sinerkia_jitsi_meet.email_template_edi_jitsi_meet"
            )
            self.env["mail.template"].sudo().browse(template.id).send_mail(
                res.id
            )
